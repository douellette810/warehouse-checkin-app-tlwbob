...
interface ChargeMaterialsStepProps {
...
export default function ChargeMaterialsStep({
  formData,
  updateFormData,
  materials,
  onNext,
  onBack,
}: ChargeMaterialsStepProps) {
  ...
  const addMaterialRow = () => {
    const newMaterials = [...formData.chargeMaterials, { materialId: '', materialName: '', quantity: '', measurement: '' }];
    updateFormData({ chargeMaterials: newMaterials });
  };

  const removeMaterialRow = (index: number) => {
    const newMaterials = formData.chargeMaterials.filter((_, i) => i !== index);
    updateFormData({ chargeMaterials: newMaterials });
  ...
  const handleMaterialSelect = (material: Material) => {
    if (selectedIndex !== null) {
      const newMaterials = [...formData.chargeMaterials];
  ...
  const openQuantityInput = (index: number) => {
    ...
    setQuantityValue(formData.chargeMaterials[index].quantity);
  ...
  const isFormValid = () => {
    ...
    if (formData.chargeMaterials.length === 0) {
      return true;
    }
    return formData.chargeMaterials.every(item => item.materialName && item.quantity);
  };

  return (
    <View style={styles.container}>
      <Text style={styles.sectionTitle}>Charge Materials</Text>
      <Text style={styles.sectionDescription}>
        Please add charge materials, including mixed electronics not entered in previous step
      </Text>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            {quantityInputIndex !== null && formData.chargeMaterials[quantityInputIndex] && (
              <Text style={styles.modalSubtitle}>
                Unit: {formData.chargeMaterials[quantityInputIndex].measurement || 'Select material first'}
              </Text>
            )}
            <TextInput
              style={styles.input}
